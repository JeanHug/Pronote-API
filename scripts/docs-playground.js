...
(function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    call("/api/v1/scrape-pronote", {
      ...
      var attempts = 0;
      function poll() {
        if (res.status !== 202 || !jobId || attempts >= 40 || controller.signal.aborted) return finish(res);
        attempts++;
        show("202", "Extraction en cours (" + attempts + "). Le navigateur ne garde plus la connexion ouverte.");
