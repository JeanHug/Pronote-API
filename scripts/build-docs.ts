...
const errorCodes: [code: string, http: string, description: string][] = [
  ...
  ['EDUCONNECT_NETWORK_BLOCKED', '503', 'EduConnect a répondu « Accès perturbé » : le Ministère filtre les IP hébergeur/VPN du moteur. Les identifiants n’ont pas été évalués. Le moteur doit sortir par une IP française (secret EDUCONNECT_PROXY).'],
  ['EDUCONNECT_AUTH_FAILED', '401', 'Identifiant ou mot de passe EduConnect refusé par le formulaire EduConnect.'],
  ['EDUCONNECT_MFA_REQUIRED', '409', 'EduConnect demande une validation supplémentaire ; elle n’est jamais contournée.'],
  ['EDUCONNECT_ACTION_REQUIRED', '409', 'Action requise sur EduConnect (activation du compte, changement de mot de passe).'],
  ['EDUCONNECT_NAVIGATION', '502', 'Parcours EduConnect interrompu ; l’étape atteinte est indiquée dans le message.'],
...
${table(['Champ', 'Type', 'Description'], [
  ['username', 'string · requis', 'Identifiant ENT77 ou EduConnect. 200 caractères maximum.'],
  ['password', 'string · requis', 'Mot de passe correspondant. 200 caractères maximum.'],
  ['provider', 'string · facultatif', 'ent77 par défaut, ou educonnect. EduConnect suit le lien officiel de l’établissement puis revient sur Pronote.'],
  ['account', 'string · facultatif', 'student par défaut, ou parent. Utilisé uniquement pour choisir le profil EduConnect.'],
...
<div class="frow">
<label>Connexion<select id="provider"><option value="ent77">ENT77</option><option value="educonnect">EduConnect</option></select></label>
