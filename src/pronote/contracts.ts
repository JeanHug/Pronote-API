...
export type AuthMode = 'auto' | 'local' | 'educonnect' | 'educonnect_eleve' | 'educonnect_parent';
...
export interface ExtractionResult {
  ...
  timestamp: string;
  durationMs: number;
  authentication: { ent: boolean; pronote: boolean; authMode?: AuthMode };
...
export function validateCredentials(body: unknown): Credentials {
  ...
  const rawSession = typeof value.sessionCookie === 'string' ? value.sessionCookie.trim() : '';
  const username = typeof value.username === 'string' ? value.username.trim() : '';
  const password = typeof value.password === 'string' ? value.password : '';
  if (!rawSession && (!username || !password || username.length > 200 || password.length > 200)) {
    throw new ApiError('INVALID_REQUEST', 400, 'validation', 'username et password sont requis (200 caractères maximum), ou fournissez sessionCookie.');
  }
  ...
  if (entUrl !== DEFAULT_ENT && entUrl !== 'https://ent77.seine-et-marne.fr/') throw new ApiError('UNSUPPORTED_ENT', 400, 'validation', 'Cette version prend en charge ENT77 / EduConnect uniquement.');
  ...
  // Normalize authMode / authType
  const rawMode = (typeof value.authMode === 'string' ? value.authMode : typeof value.authType === 'string' ? value.authType : typeof value.provider === 'string' ? value.provider : 'auto').toLowerCase().trim();
  let authMode: AuthMode = 'auto';
  if (rawMode === 'local' || rawMode === 'ent77') authMode = 'local';
  else if (rawMode === 'educonnect' || rawMode === 'educonnect_eleve' || rawMode === 'student' || rawMode === 'eleve') authMode = 'educonnect_eleve';
  else if (rawMode === 'educonnect_parent' || rawMode === 'parent' || rawMode === 'relative') authMode = 'educonnect_parent';
  else if (rawMode === 'auto') authMode = 'auto';

  let proxyUrl: string | undefined;
  if (typeof value.proxyUrl === 'string' && value.proxyUrl.trim()) {
    try {
      const p = new URL(value.proxyUrl.trim());
      if (['http:', 'https:'].includes(p.protocol)) proxyUrl = p.href;
    } catch { /* ignored */ }
  }

  const sessionCookie = rawSession && rawSession.length <= 4096 ? rawSession : undefined;
  return { username, password, authMode, pronoteUrl: url.href, entUrl, modules: [...new Set(modules)] as ModuleName[], proxyUrl, sessionCookie };
}
...
export function assertNoCredentials(value: unknown): void {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (/^(username|password|pass|ent_?id|ent_?pass|authorization|ticket|token|cookie)$/i.test(key)) throw new ApiError('UNSAFE_RESULT', 500, 'serialization', 'Une donnée de connexion a été bloquée.');
